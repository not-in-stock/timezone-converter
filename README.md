...
5. Clone this repo and open a terminal in the `tz-converter` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/tz_converter/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns tz-converter.example
  (:require [tz-converter.config :as config])
